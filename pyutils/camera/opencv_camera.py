...
from .types import Image, Camera, Frame, CameraOptions
...
class OpenCvCamera(Camera):
    ...
    def image_size(self) -> Size2di:
        assert isinstance(self.__image_size, Size2di), "not initialized"
        return self.__image_size
        
    @property
    def fps(self) -> int:
        assert isinstance(self.__fps, int), "not initialized"
        return self.__fps
    ...
    def __setup_if_necessary(self, cap:cv2.VideoCapture) -> None:
        if self.__image_size is None:
            self.__image_size = _get_image_size(cap)
        else:
            _set_image_size(cap, self.__image_size)
            
        if self.__fps is None:
            self.__fps = _get_fps(cap)
        else:
            _set_fps(cap, self.__fps)
...
class OpenCvImageCapture(SyncableImageCapture):
    ...
    def __init__(self, camera:OpenCvCamera, capture:cv2.VideoCapture, init_frame_index:int=1) -> None:
        ...
        camera._OpenCvCamera__setup_if_necessary(capture)
