...
class EventNodeImpl(EventNode):
    __slots__ = ( '__handler', '__queue', )
    
    def __init__(self, handler:Optional[EventProcessor]=None) -> None:
    ...
    def handle_event(self, ev:Any) -> None:
        out_evs:Iterable[Any] = self.__handler.process(ev) if self.__handler else [ev]
        for out_ev in out_evs:
            self.__queue.publish_event(out_ev)
